...
exports.dynamicQuery05 = dynamicQuery05;
...
function dynamicQuery05(client, params) {
    return __awaiter(this, void 0, void 0, function* () {
        const { sql, paramsValues } = buildSql(params);
        return client.query({ text: sql, rowMode: 'array', values: paramsValues })
            .then(res => res.rows.map(row => mapArrayToDynamicQuery05Result(row, params === null || params === void 0 ? void 0 : params.select)));
    });
}
function buildSql(queryParams) {
    ...
    const withSql = withFragments.length > 0
        ? `WITH${os_1.EOL}${withFragments.join(`,${os_1.EOL}`)}${os_1.EOL}`
        : '';
...
function mapArrayToDynamicQuery05Result(data, select) {
    const result = {};
    let rowIndex = -1;
    if (!select || select.id === true) {
        rowIndex++;
...
//# sourceMappingURL=dynamic-query05.js.map