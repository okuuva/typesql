...
function getInParameterList(ctx) {
    const result = collectContextsOfType(ctx, PostgreSQLParser_1.C_expr_exprContext).filter(c_expr => c_expr.PARAM());
...
function checkParamterNullability(column, traverseResult) {
    if (isParameter(column.column_name)) {
        ...
        if (fun_expr) {
            ...
            if (func_application) {
                ...
                if (is_json_agg(func_application)) {
                    const filter_expr = (_a = fun_expr.filter_clause()) === null || _a === void 0 ? void 0 : _a.a_expr();
                    return traverse_json_agg(func_application, Object.assign(Object.assign({}, context), { filter_expr }), traverseResult);
                }
...
function traverse_joins(joinList, context, traverseResult) {
    ...
    joinList.forEach((join, index) => {
        ...
        const isLeftJoin = (joinType === null || joinType === void 0 ? void 0 : joinType.LEFT()) != null;
        const nullableColumns = joinColumns.map(col => (Object.assign(Object.assign({}, col), { original_is_nullable: col.is_nullable, is_nullable: isLeftJoin && checkLeftJoinIsNullable(join, subsequentJoints, joinColumns) ? true : col.is_nullable })));
...
function paramIsList(c_expr) {
    ...
    const in_expr_list = expr_list === null || expr_list === void 0 ? void 0 : expr_list.parentCtx;
...
function traverseInsertstmt(insertstmt, dbSchema) {
    ...
    traverseSelectstmt(selectstmt, Object.assign(Object.assign({}, context), { fromColumns: insertColumnsList }), traverseResult);
...
function traverseDeletestmt(deleteStmt, dbSchema, traverseResult) {
    ...
    const whereExpr = (_a = deleteStmt.where_or_current_clause()) === null || _a === void 0 ? void 0 : _a.a_expr();
    if (whereExpr) {
...
function traverseUpdatestmt(updatestmt, traverseContext, traverseResult) {
    ...
    const result = {
        queryType: 'Update',
        multipleRowsResult: false,
        parametersNullability: traverseResult.parameters.slice(0, parametersBefore).map(param => (Object.assign({ isNotNull: param.isNotNull }, addConstraintIfNotNull(param.checkConstraint)))),
        columns: returninColumns,
        parameterList: paramIsListResult,
        whereParamtersNullability: whereParameters.map(param => (Object.assign({ isNotNull: param.isNotNull }, addConstraintIfNotNull(param.checkConstraint))))
    };
    if (returning_clause) {
        result.returning = true;
    }
...
function traverse_insert_column_item(insert_column_item, fromColumns) {
...
function getCheckedKeys_a_expr_between(a_expr_between) {
...
function getCheckedKey_a_expr_in(a_expr_in) {
    var _a, _b, _c;
...
//field or null
function getCheckedUniqueColumn(a_expr_like) {
    var _a, _b;
...
function traverse_columnElem(columnElem, fromColumns, traverseResult) {
